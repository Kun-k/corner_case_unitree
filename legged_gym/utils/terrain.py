...
class Terrain:
    def __init__(self, cfg: LeggedRobotCfg.terrain, num_robots, choice=2) -> None:
        ...
        if self.type == "trimesh":
            print("self.height_field_raw.shape[0]", self.height_field_raw.shape[0], "self.height_field_raw.shape[1]",
                  self.height_field_raw.shape[1])
    ...
    def make_terrain(self, choice, difficulty):
        ...
        # print("choice: ", choice, "proportions: ", self.proportions) [0.1, 0.2, 0.55, 0.8, 1.0]
        if choice < self.proportions[0]:
            ...
            terrain_utils.pyramid_sloped_terrain(terrain, slope=slope, platform_size=3.)
            terrain_utils.random_uniform_terrain(terrain, min_height=-0.00, max_height=0.05, step=0.005,
    ...
    def make_terrain_custom(self, choice, difficulty):
        terrain = terrain_utils.SubTerrain("terrain",
                                           width=self.width_per_env_pixels,
                                           length=self.width_per_env_pixels,
                                           vertical_scale=self.cfg.vertical_scale,
        ...
        # print("choice: ", choice, "proportions: ", self.proportions) [0.1, 0.2, 0.55, 0.8, 1.0]
        if choice == 1:
            # print("choice:  pyramid sloped terrain")
            # slope *= -1
            terrain_utils.pyramid_sloped_terrain(terrain, slope=slope, platform_size=3.)
        elif choice == 2:
            ...
            # print("choice:  stepping stones terrain")
            terrain_utils.stepping_stones_terrain(terrain, stone_size=stepping_stones_size,
                                                  stone_distance=stone_distance, max_height=0., platform_size=4.)
        elif choice == 6:
