...
from legged_gym.envs.go2_stair_navigation.go2_terrain_config import GO2TerrainCfg
...
class Go2TerrainRobot(BaseTask):
    ...
    def _post_physics_step_callback(self):
        """Refresh target cmd and optionally switch to next configured target point."""
    ...
    def _resample_commands(self, env_ids=None):
        ...
        if env_ids is None:
            ...
            if target_cfg["switch_on_reach"]:
                ...
                switch_mask |= distance < self.target_reach_radius
            if self.target_switch_interval_steps > 0:
    ...
    def _init_buffers(self):
        ...
        # Obs uses world target cmd, so keep x/y unscaled and heading scaled.
        ...
        self.cmd_world = torch.zeros(self.num_envs, 3, device=self.device, dtype=torch.float32)
        self.target_points = torch.tensor(
            self.runtime_cfg["targets"]["target_cmd_list"],
            dtype=torch.float32,
            device=self.device,
        )

        self.num_target_points = int(self.target_points.shape[0])
        self.target_list_index = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def _create_envs(self):
        ...
        print("env origins: ", self.env_origins)
    ...
    def _parse_cfg(self):
        ...
        self.cfg.rewards.scales.tracking_xy = self.runtime_cfg["rewards"]["tracking_xy_scale"]
        self.cfg.rewards.scales.tracking_ang_vel = self.runtime_cfg["rewards"]["tracking_ang_vel"]
        self.cfg.rewards.scales.speed = self.runtime_cfg["rewards"]["speed_scale"]

        self.cfg.rewards.tracking_xy_sigma = self.runtime_cfg["rewards"]["tracking_xy_sigma"]
        self.cfg.rewards.tracking_ang_vel_sigma = self.runtime_cfg["rewards"]["tracking_ang_vel_sigma"]
        self.cfg.rewards.max_speed_reward = self.runtime_cfg["rewards"]["max_speed_reward"]

        self.target_reach_radius = self.runtime_cfg["targets"]["target_reach_radius"]

        self.reward_scales = class_to_dict(self.cfg.rewards.scales)

        target_switch_interval_s = self.runtime_cfg["targets"]["target_switch_interval_s"]
        self.target_switch_interval_steps = int(target_switch_interval_s / self.dt) if target_switch_interval_s > 0 else 0
        print("reward scales: ", self.reward_scales)

        self.cfg.commands.ranges.target_x = np.array(self.runtime_cfg["targets"]["target_x_range"])
        self.cfg.commands.ranges.target_y = np.array(self.runtime_cfg["targets"]["target_y_range"])
    ...
    def _load_runtime_cfg(self):
        ...
        # Support list entries as [x, y] (heading defaults to 0) or [x, y, heading_deg]
        norm_list = []
        for cmd in data["targets"]["target_cmd_list"]:
            norm_list.append([float(cmd[0]), float(cmd[1]), float(cmd[2]) * pi / 180.0])
        data["targets"]["target_cmd_list"] = norm_list
        ...
        *************************************************************************'''
        ...
        if self.record_video:
            ...
            if not self.camera_list:
                ...
                for i in range(self.num_envs):
                    self.camera_list.append(self.gym.create_camera_sensor(self.envs[i], camera_props))
            
            camera_frames_step_list = []
            for i in range(self.num_envs):
                camera_frames_step_list.append([])

            for i in range(self.num_envs):
                cam_pos = gymapi.Vec3(*(x + y for x, y in zip(self.root_states[i, 0:3].tolist(), [3., -3., 2.] ))) # self.cfg["viewer"]["pos"]
                cam_target = gymapi.Vec3(*self.root_states[i, 0:3].tolist()) # self.cfg["viewer"]["ref_env"]
                self.gym.set_camera_location(self.camera_list[i], self.envs[i], cam_pos, cam_target) # self.cfg["viewer"]["ref_env"]
                self.gym.render_all_camera_sensors(self.sim)
                img = self.gym.get_camera_image(self.sim, self.envs[i], self.camera_list[i], gymapi.IMAGE_COLOR) # self.cfg["viewer"]["ref_env"]
                camera_frames_step_list[i].append(img.reshape(img.shape[0], -1, 4))

        return camera_frames_step_list if self.record_video else None
    ...
    def _reward_tracking_xy(self):
        """Track target xy position in world frame."""
        xy_error = torch.sum(torch.square(self.cmd_world[:, :2] - self.base_pos[:, :2]), dim=1)
        return torch.exp(-xy_error / self.cfg.rewards.tracking_xy_sigma)

    def _reward_speed(self):
        """Encourage faster progress speed along the direction to target."""
        target_vec = self.cmd_world[:, :2] - self.base_pos[:, :2]
        distance = torch.norm(target_vec, dim=1).clamp(min=1e-6)
        target_dir = target_vec / distance.unsqueeze(1)
        progress_speed = torch.sum(self.base_lin_vel[:, :2] * target_dir, dim=1)
        return torch.clamp(progress_speed, min=0.0, max=self.cfg.rewards.max_speed_reward)
