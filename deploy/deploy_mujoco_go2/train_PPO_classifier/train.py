...
class GatedPPO(PPO):
    """PPO variant that executes random action when classifier score is below threshold.

    Only RL-selected transitions are inserted into rollout buffer.
    """
    ...
    def collect_rollouts(self, env, callback, rollout_buffer, n_rollout_steps):
        ...
        while n_steps < n_rollout_steps:
            ...
            if env_steps > n_rollout_steps * 200:
                raise RuntimeError(
                    "Gated PPO could not collect enough RL transitions. "
                    "Try lowering classifier_gate.threshold."
                )
            ...
            keep = use_rl_mask & self._get_keep_mask(infos)
            if np.any(keep):
                rollout_buffer.add(
                    self._last_obs[keep],
                    actions[keep],
...
def train_ppo(go2_cfg, terrain_cfg, total_timesteps=20000, max_episode_steps=35, log_dir="train_terrain_logs", device="auto", learning_rate=3e-4, batch_size=256, n_steps=2048, n_epochs=10, gamma=0.99, gae_lambda=0.95, clip_range=0.2, ent_coef=0.0, vf_coef=0.5, max_grad_norm=0.5, seed=0, plot_save_every_steps=2000, plot_smooth_window=20, checkpoint_every_steps=10000, preload_model_path="", failure_pkl_name="train_failure_chains.pkl", failure_flush_every_episodes=50, classifier_gate=None, classifier_threshold=0.5, consecutive_fail_keep_k: int = 0):
    ...
    model_cls = GatedPPO if gated_mode else FilteredPPO
    model_kwargs = dict(
        policy="MlpPolicy",
        env=vec_env,
        verbose=1,
    ...
    if gated_mode:
        ...
        model_kwargs.update(consecutive_fail_keep_k=int(consecutive_fail_keep_k))
