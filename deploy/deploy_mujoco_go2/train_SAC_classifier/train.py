...
class GateFilteredReplayBuffer(ReplayBuffer):
    """ReplayBuffer that only stores transitions with infos[i]['use_rl_action']=True."""
    ...
    def __init__(self, *args, consecutive_fail_keep_k: int = 0, **kwargs):
        ...
        # Per-env counter for consecutive fail+stuck frames.
    ...
    def add(self, obs, next_obs, action, reward, done, infos):
        ...
        # Apply consecutive fail+stuck trimming during sampling collection.
        ...
        if keep.size == 0 or not np.any(keep):
            return
        super().add(obs[keep], next_obs[keep], action[keep], reward[keep], done[keep], [infos[i] for i in np.where(keep)[0]])
...
class GatedSAC(SAC):
    ...
    def __init__(self, *args, gate: ClassifierGate, gate_threshold: float = 0.5, min_rl_buffer_size_for_update: int = 1000, consecutive_fail_keep_k: int = 0, **kwargs):
        kwargs.setdefault("replay_buffer_class", GateFilteredReplayBuffer)
...
def preload_replay_buffer_from_pkl(model, pkl_paths, reward_cfg=None, consecutive_fail_keep_k: int = 0):
    ...
    print(
        f"[train_SAC_classifier] replay preload inserted transitions: {inserted}, "
...
def train_sac(go2_cfg, terrain_cfg,
    ...
    model_kwargs = dict(
        ...
        gamma=float(gamma),
        seed=int(seed),
        replay_buffer_class=GateFilteredReplayBuffer,
    ...
    if gated_mode and hasattr(model, "rl_action_count"):
        print(
